...
public class Rigid_Bunny_by_Shape_Matching : MonoBehaviour
{
    ...
    Vector3 N2 = new Vector3(-1.0f, 0, 0);
    ...
    Vector3 Collision_Impulse(Vector3 P, Vector3 N, Vector3 v_in, Vector3 collision_points)
    {
		if (Vector3.Dot(v_in, N) >= 0.0f) return v_in;
        Vector3 vn = Vector3.Dot(v_in, N) * N;
        Vector3 vt = v_in - vn;
        Vector3 v_N_new = -restitution * vn;
        float a = Math.Max(1.0f - friction * (1.0f + restitution) * vn.magnitude / vt.magnitude, 0.0f);
        Vector3 v_T_new = a * vt;
        return v_N_new + v_T_new;
    }
    ...
    void Collision(float inv_dt)
	{
		...
		for(int i = 0; i < Q.Length; i++)
		{
			if (Vector3.Dot(X[i] - P1, N1) < 0.0f)
			{
				V[i] = Collision_Impulse(P1, N1, V[i], X[i]);
            }
			else if(Vector3.Dot(X[i] - P2, N2) < 0.0f)
			{
                V[i] = Collision_Impulse(P2, N2, V[i], X[i]);
            }
		}
	}
    ...
    void Update()
    {
        ...
        if (launched)
		{
            ...
            {
                a1 = Matrix_Plus(a1, vector3x1dotvector1x3(Y[i] - c, Q[i]));
            }
			a1[3, 3] = 1.0f;
